...
import { AutorLivros } from "../models";
...
class AutorLivroController {
    ...
    public async create(req: Request, res: Response): Promise<Response> {
        ...
        try {
            const document = new AutorLivros({ livro, autor });
    ...
    public async list(req: Request, res: Response): Promise<Response> {
        ...
        try {
            ...
            const objects = await AutorLivros.find({"autor":_id})
                .populate("autor")
                .populate("livro")
                .select("livro")
                .sort({ titulo: "asc" });
    ...
    public async delete(req: Request, res: Response): Promise<Response> {
        ...
        try {
            const object = await AutorLivros.findByIdAndDelete(_id);
            if (object) {
    ...
    public async update(req: Request, res: Response): Promise<Response> {
        ...
        try {
            ...
            const document = await AutorLivros.findById(id);
            if (!document) {
                return res.json({ message: "Registro inexistente!" });
            }
            // atualiza os campos
